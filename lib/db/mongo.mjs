...
//
// If MONGODB_URI is unset, isEnabled() is false and every caller falls back to
// the committed JSON it read before. That is the whole safety story of this
// migration: nothing breaks when Mongo is missing, it just gets slower and
// read-only, exactly as it behaved when GH_TOKEN was missing.
...
// Run `fn` against Mongo, and if Mongo is disabled OR unreachable, return the
// result of `fallback()` instead. This is the single place the "nothing breaks"
// guarantee is implemented, so the failure decision stays consistent across
// every endpoint rather than being re-litigated per call site.
export async function withFallback(fn, fallback, label = 'mongo') {
  if (!isEnabled()) return fallback();
  try {
    return await fn(await getDb());
  } catch (err) {
    console.error(`[${label}] Mongo unavailable, falling back to JSON:`, err?.message);
    return fallback();
  }
}
